...
export default function Form() {
    ...
    // Skill Field Handler
    const handleSkiils = (e: React.ChangeEvent<HTMLInputElement>) => {
        const { name, value } = e.target
        setSkillsec((sec) => ({
    ...
    return (
        <div className="px-8 sm:px-20 font-serif 2xl:px-20">
            ...
            <form onSubmit={formHandleSubmit} style={{ background: "url(bgForm.jpg)" }} className="bg-cover bg-center border-2 border-gray-500 bg-gray-400 py-5 md:py-10 lg:py-20 px-3 md:px-8 lg:px-16 xl:px-5">
                ...
                {/* Skills */}
                <SkillInfo skillsec={skillsec} onSkillsChange={handleSkiils} />
            ...
            <div className="h-auto py-8">
                <div className="bg-gray-400 p-8 space-y-3 lg:space-y-5 xl:space-y-8 border-2 border-gray-500">
                    ...
                    <div className="space-y-3 lg:space-y-5 xl:space-y-8 lg:px-10">
                    ...
                    <div className="flex space-x-10 items-center px-4 lg:px-16 font-bold text-sm md:text-base lg:text-lg xl:text-xl 2xl:text-3xl">
                        <label>[From-To]</label>
                        <label>[Job Position]</label>
                        <label>[Company Name]</label>
                    </div>
                    <div className="flex space-x-10 items-center lg:mx-4">
                        {/* First Field */}
                        <p className="text-sm md:text-base lg:text-lg xl:text-xl"> {experience.experienceFromTo1}</p>
                        <p className="text-sm md:text-base lg:text-lg xl:text-xl">{experience.experiencePosition1}</p>
                        <p className="text-sm md:text-base lg:text-lg xl:text-xl">{experience.experienceCompany1}</p>
                    </div>

                    <div className="flex space-x-10 items-center lg:mx-4">
                        {/* Second Field */}
                        <p className="text-sm md:text-base lg:text-lg xl:text-xl">{experience.experienceFromTo2}</p>
                        <p className="text-sm md:text-base lg:text-lg xl:text-xl">{experience.experiencePosition2}</p>
                        <p className="text-sm md:text-base lg:text-lg xl:text-xl">{experience.experienceCompany2}</p>
                    </div>

                    <div className="flex space-x-10 items-center lg:mx-4">
                        {/* Third Field */}
                        <p className="text-sm md:text-base lg:text-lg xl:text-xl">{experience.experienceFromTo3}</p>
                        <p className="text-sm md:text-base lg:text-lg xl:text-xl">{experience.experiencePosition3}</p>
                        <p className="text-sm md:text-base lg:text-lg xl:text-xl">{experience.experienceCompany3}</p>
                    </div>
                    </div>
